...
from calculate_result import my_metric
...
def write_fairness_trial(output, data_dir, post_fix = "", shiyan = 0):
    #g_date_format = "%Y-%m-%d-%H:%M:%S" # format of output
    #g_date_format = "%S"  # format of output
    #now = datetime.datetime.now().strftime(g_date_format)
    #data_file = data_dir + shiyan

    data_file = data_dir + str(shiyan[0])
    #data_file += "test.txt"
    #print(data_file)
...
def gonzales_k_center(df, n_clusters):
    ...
    #print(df)
    #print(df.iloc[max_dist_ind])
    #print(df.iloc[max_dist_ind].tolist())
...
def arya_etal_k_median(df, n_clusters, num_trial = 1):
    ...
    #return best_cost, best_pred, sort_centers(actual_centers)
    return best_pred, sort_centers(actual_centers)
...
def vanilla_clustering(df, num_clusters, clustering_method):
    if clustering_method == "kmedoids":
        ...
        init_medoids = random.sample(range(0, len(df.values)), num_clusters)
        kmedoids_instance = kmedoids(df.values, init_medoids, metric=metric)
        ...
        cluster_centers = []
        for i in medoids:
            cluster_centers.append(df.values[i])
        pred = []
        ...
        return pred, sort_centers(cluster_centers)
